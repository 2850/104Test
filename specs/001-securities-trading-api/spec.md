...
4. **假設** 外部資料來源不可用，**當** 使用者查詢股票資訊時，**則** 系統返回快取資料（如果在可接受的過時閾值內可用）並附上資料新鮮度的通知
...
- **擷取外部市場數據時網路逾時**：如果可用且在可接受的過時範圍內（例如，< 1 分鐘），系統應返回快取資料，或返回服務不可用錯誤並提供重試指引
- **同一股票的並行委託單**：系統必須處理同時提交的委託單而不會造成資料損壞，確保每個委託單都獨立記錄
...
- **FR-007**: 系統必須對股票資料進行快取，並具有可配置的存活時間，以減少外部 API 呼叫並改善回應時間
- **FR-008**: 系統必須在股票資料無法取得或已過時時返回適當的錯誤訊息
...
- **FR-017**: 系統必須持久化保存所有委託單資料以防止資料遺失

#### 資料管理

- **FR-018**: 系統必須維護可用股票的永久性目錄，包括代碼和公司名稱
- **FR-019**: 系統必須按存取模式分類資料儲存（熱資料：即時價格；暖資料：最近委託單；冷資料：歷史記錄）
- **FR-020**: 系統必須支援分開的讀寫操作以啟用效能最佳化

#### API 標準

- **FR-021**: 系統必須通過 RESTful HTTP 端點公開所有功能，遵循標準慣例（GET 用於查詢，POST 用於建立）
- **FR-022**: 系統必須以 JSON 格式返回回應
- **FR-023**: 系統必須使用標準 HTTP 狀態碼（200 表示成功，400 表示客戶端錯誤，404 表示找不到，500 表示伺服器錯誤）
- **FR-024**: 系統必須提供結構化的錯誤回應，包括錯誤代碼和易讀的訊息
- **FR-025**: 系統必須通過互動式文件介面支援 API 文件
- **FR-026**: 系統必須驗證所有傳入的請求有效負載並返回詳細的驗證錯誤

#### 效能和可靠性

- **FR-027**: 系統必須能夠處理至少 100 個併發使用者請求而不會出現明顯的效能降低
- **FR-028**: 系統必須為所有 API 呼叫實作請求日誌記錄，包括請求參數、回應狀態和執行時間
- **FR-029**: 系統必須為所有系統故障和例外實作錯誤日誌記錄
- **FR-030**: 系統必須提供健康檢查端點以監控系統可用性
